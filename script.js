// =========================================================================
// script.js - FINAL COMPLETO - Baseado no Original + Estrutura + TODOS Caminhos Relativos dessa porra
// =========================================================================
...
// =========================================================================
// FUNÇÕES INTERNAS (Exatamente como no seu original)
// =========================================================================
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (playerMusica) {
        ...
        // Listener dos botões Favoritar e Lista (COMO NO SEU ORIGINAL)
        // Estes listeners são adicionados AQUI, dentro do DOMContentLoaded,
        // garantindo que os botões existam antes de adicionar o listener.
    ...
    // Inicializa Lista de Músicas (COMO NO SEU ORIGINAL)
    // Esta função já está definida globalmente, mas a chamada inicial precisa
    // ocorrer após o DOM carregar para encontrar 'listaMusicas'.
    if (document.getElementById('listaMusicas')) {
         atualizarListaMusicas();
         // selecionarMusica(1); // REMOVIDO do DOMContentLoaded original - pode causar play indesejado
    ...
    if(carrosselContainer){
        ...
        if(carrossel) {
             ...
             // Gerenciar cliques (COMO NO SEU ORIGINAL)
             document.querySelectorAll('.titulo-item').forEach((item) => {
                item.addEventListener('click', (e) => {
                    pausarCarrossel();
                    // Extrai o ID do atributo onclick="abrirJanelaTitulo(ID)"
                    const onclickAttr = e.currentTarget.getAttribute('onclick');
                    if (onclickAttr) {
                        const idMatch = onclickAttr.match(/abrirJanelaTitulo\((\d+)\)/);
                        if (idMatch && idMatch[1]) {
                             abrirJanelaTitulo(parseInt(idMatch[1])); // Chama a função global
                        }
                    }
                });
            });
...
// Definições duplicadas de loadSection removidas.

console.log("Script.js totalmente carregado (Baseado 100% no original + Estrutura + CAMINHOS RELATIVOS APLICADOS).");
