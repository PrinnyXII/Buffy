// =========================================================================
// script.js - Versão Final Baseada no Original com Caminhos Relativos
// =========================================================================
...
// =========================================================================
// Para Player Isaac (serão inicializadas no DOMContentLoaded)
let playerMusica, audio, audioSource, progressBar, tempoAtual, tempoTotal;
let musicaTocando = false;
// Para Carrossel Títulos (serão inicializadas no DOMContentLoaded)
let carrosselTitulosInterval;
let pausarCarrosselTitulosFunc, iniciarCarrosselTitulosFunc;
// Para Selos
let chaveAtual = 0;
// Para Carrossel Bênçãos
...
const listaDeMusicas = [ // Nome original da variável
    {
        id: 1,
        ...
        // Mantido pois é do repositório Cueinhah/Painel-de-Buffy
        ...
        link: "assets/CryingAlone-Nowhere.mp3", // Confirmar se está em /assets/ no repo Buffy
    }
];
const storageKey = 'musicasFavoritadas'; // Nome original
let musicasFavoritadas = JSON.parse(localStorage.getItem(storageKey)) || {}; // Nome original

// --- ATRIBUTOS ---
const atributos = { // Nome original
    ...
    unknown: { total: 100, porcentagem: 50 } // Chave era 'unknown' no seu objeto original
};

// --- SELOS ---
// !!! VERIFIQUE SE ESTES CAMINHOS CORRESPONDEM À ESTRUTURA NO REPO 'Buffy' !!!
const chaves = [ // Nome original
    ...
    { id: 5, nome: "Key of Pendragon", descricao: "Nenhuma informação...",
      ...
      // Link original: assets/Recursos/Key of Pendragon.png (Já era relativo)
      icone: "assets/Recursos/Key of Pendragon.png", // Confirmar localização em /assets/Recursos/
      detalhes: "..."},
    ...
    { id: 7, nome: "Key of Isaac's Heart", descricao: "Nenhuma informação...",
      // Link original: assets/Recursos/Key of Isaac's Heart.png (Já era relativo)
      item: "assets/Recursos/Key of Isaac's Heart.png", // Confirmar localização em /assets/Recursos/
...
const estadosIniciais = { // Nome original
...
// --- TÍTULOS ---
function abrirJanelaTitulo(id) { if(typeof pausarCarrossel ==='function')pausarCarrossel();const j=document.getElementById(`janelaTitulo${id}`);if(j)j.style.display='block';} // pausarCarrossel é global no original
function fecharJanelaTitulo(id) { const j=document.getElementById(`janelaTitulo${id}`);if(j){j.style.display='none';if(typeof iniciarCarrossel==='function')iniciarCarrossel();}} // iniciarCarrossel é global no original
...
// =========================================================================
// FUNÇÕES INTERNAS (Não chamadas diretamente pelo HTML, baseadas no original)
// =========================================================================
...
function atualizarBarra(idB,idT,p,idS=null){const b=document.getElementById(idB);const t=document.getElementById(idT);if(!b||!t)return;b.style.width=`${p}%`;t.textContent=`${p}%`;let c;if(p<=20)c='darkred';else if(p<=40)c='#FF9100';else if(p<=60)c='#00D19A';else if(p<=80)c='#D622EF';else c='#6222EF';b.style.backgroundColor=c;if(idS){const sE=document.getElementById(idS);if(sE){let tS;if(p<=20)tS='Infame..';else if(p<=40)tS='Desprezado..';else if(p<=60)tS='Ambíguo..';else if(p<=80)tS='Respeitado..';else tS='Renomado..';sE.textContent=tS;}}} // Global do original
function iniciarCarrossel() { const c=document.querySelector('.carrossel-imagens');if(!c)return;clearInterval(carrosselTitulosInterval);carrosselTitulosInterval=setInterval(()=>{if(!c){clearInterval(carrosselTitulosInterval);return;}c.scrollLeft+=1;if(c.scrollLeft>=c.scrollWidth-c.offsetWidth){c.scrollLeft=0;}},30);} // Global do original
function pausarCarrossel() { clearInterval(carrosselTitulosInterval);} // Global do original
function atualizarDestaqueCirculo(id) {document.querySelectorAll(".circulo-pequeno").forEach((c,i)=>{c.style.boxShadow=(i+1===id)?"0 0 10px 3px #FFD700":"none";});} // Global do original
function toggleEstado(id) {const c=document.getElementById(id);if(c)c.classList.toggle('ativo');} // Global do original
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // --- Inicializa Player Isaac (Elementos do index.html) ---
    playerMusica = document.querySelector('.player-musica-isaac'); // Nome original da variável global
    if (playerMusica) {
        audio = playerMusica.querySelector('#audio-player'); // Nome original
        if (audio) { audioSource = audio.querySelector('source'); } // Nome original
        progressBar = playerMusica.querySelector('#progress-bar'); // Nome original
        tempoAtual = playerMusica.querySelector('#tempo-atual'); // Nome original
        tempoTotal = playerMusica.querySelector('#tempo-total'); // Nome original

        // Listeners internos do player (usando nomes de variáveis originais)
        ...
        if (audio) {
            audio.addEventListener('timeupdate', () => {
                if (tempoAtual && !isNaN(audio.currentTime)) tempoAtual.textContent = formatarTempo(audio.currentTime);
                if (progressBar && audio.duration && !isNaN(audio.duration) && audio.duration > 0) progressBar.value = (audio.currentTime / audio.duration) * 100;
            });
            audio.addEventListener('loadedmetadata', () => {
                 if (tempoTotal && !isNaN(audio.duration) && audio.duration > 0) tempoTotal.textContent = formatarTempo(audio.duration);
                 else if(tempoTotal) tempoTotal.textContent = "0:00";
            });
            // Eventos 'ended', 'play', 'pause' para atualizar `musicaTocando` e o botão
        ...
        if (document.getElementById('listaMusicas')) {
            atualizarListaMusicas(); // Nome original
            document.getElementById('listaMusicas').style.display = 'none';
            atualizarBotaoPlay(); // Nome original
        }
    } else {
        console.warn("Elemento do Player de Música Isaac ('.player-musica-isaac') não encontrado no DOM inicial.");
    }

    // --- Atualiza Barras Status (Fama/Autoestima) ---
    atualizarBarra('barra-autoestima', 'texto-autoestima', 98); // Chama função global original
    atualizarBarra('barra-fama', 'texto-fama', 97, 'status-fama'); // Chama função global original

    // --- Configura Carrossel Títulos ---
    const carrosselEl = document.querySelector('.carrossel-imagens'); // Nome original da variável
    const carrosselContainerEl = document.querySelector('.carrossel-titulos'); // Nome original da variável
    pausarCarrosselTitulosFunc = pausarCarrossel; // Exporta função global
    iniciarCarrosselTitulosFunc = iniciarCarrossel; // Exporta função global
    if(carrosselEl && carrosselContainerEl){
        iniciarCarrossel(); // Chama função global original
        carrosselContainerEl.addEventListener('mouseover', pausarCarrossel); // Chama função global original
        carrosselContainerEl.addEventListener('mouseout', iniciarCarrossel); // Chama função global original
         // Listener de clique removido daqui, pois o onclick no HTML já chama abrirJanelaTitulo
    }
    document.querySelectorAll('.janela-titulos').forEach(addDragEventsToWindow);

    // --- Atualiza Atributos ---
    for (let atributoKey in atributos) { // Usa nome original do objeto
    ...
    // --- Inicializa Selos ---
    for (const [id, ativo] of Object.entries(estadosIniciais)) { // Usa nome original
    ...
    document.querySelectorAll(".titulo-item, .titulo-efeito, .descricao-detalhada, .detalhes-detalhados").forEach(elemento => {
         if(elemento) elemento.contentEditable = "false"; // Nome original da query
    });
    if (document.getElementById("titulo-item")) { navegar(0); } // Chama função global original
    ...
    if (diamantes.length > 0) {
        ...
        moverCarrossel(0); // Chama função global original
    }
    document.querySelectorAll('.janela-bencao').forEach(addDragEventsToWindow);

    // --- Atualiza Barra EA ---
    atualizarEA(86); // Chama função global original
    ...
    // --- Atualiza Aether ---
    atualizarAether(porcentagemAether); // Chama função global original
